...
Future<void> applyNvidiaFixes() async {
  try {
    ...
    if (!nvidiaDetected && Platform.isLinux) {
      try {
    ...
    if (nvidiaDetected) {
      appLog('⚠️ [NVIDIA] Driver NVIDIA rilevato. Applicazione fix per compatibilità...');
      ...
      if (nvidiaDriverVersion != null) {
        ...
        if (majorVersion != null && (majorVersion == 580 || majorVersion == 590)) {
          ...
          appLog('⚠️ [NVIDIA] Rilevato driver problematico (${majorVersion}xx).');
        }
      }
      if (Platform.isLinux) {
        appLog('✅ [NVIDIA] Fix Linux applicati.');
        if (isProblematicDriver) {
          appLog('   - LIBGL_ALWAYS_INDIRECT=1');
        }
      }
    }
  } catch (e) {
  ...
  if (Platform.isLinux) {
    try {
      ...
      if (waylandDisplay != null && gdkBackend != 'x11') {
        appLog('⚠️ [Wayland] Rilevato Wayland. Per migliori prestazioni, considera X11.');
      }
    ...
    try {
      ...
      appLog('⚠️ [GPU] Errore durante rilevamento GPU Windows: $e');
    }
  }
...
class _VideoConverterAppState extends State<VideoConverterApp> {
  ...
  ThemeData _buildDarkTheme() {
    ...
    return ThemeData(
      ...
      cardTheme: CardThemeData(elevation: 4, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), color: cardColor),
      ...
      dialogTheme: DialogThemeData(
        ...
        titleTextStyle: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold),
        contentTextStyle: TextStyle(color: Colors.grey[200], fontSize: 14),
      ),
      listTileTheme: ListTileThemeData(
        iconColor: Colors.grey[200],
        textColor: Colors.white,
        titleTextStyle: const TextStyle(color: Colors.white, fontSize: 16),
        subtitleTextStyle: TextStyle(color: Colors.grey[300], fontSize: 14),
      ),
