...
# Frame denoising: PyTorch DRUNet (KAIR UNetRes + drunet_color.pth) when model_path + torch;
# else OpenCV NL-means / edge-preserving (CPU) or bilateral (OpenCV CUDA).
...
def run_batch(
    ...
    use_neural = bool(model_path and Path(model_path).is_file() and _get_neural_net(model_path, device) is not None)
    if use_neural or (device in ('auto', 'cuda') and _cuda_available()):
        actual_workers = 1
    else:
    ...
    if actual_workers == 1:
        for i, p in enumerate(files, start=1):
            ...
            if process_file(p, target, noise_level, model_path, device):
                ok += 1
            report(i, ok)
    else:
        def _job(p: Path) -> bool:
            target = outdir / p.name
            return process_file(p, target, noise_level, model_path, device)
...
def main() -> int:
    p = argparse.ArgumentParser(description='FE frame denoise (KAIR DRUNet + OpenCV fallback)')
