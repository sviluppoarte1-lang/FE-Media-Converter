...
class FFmpegService {
  ...
  Future<Map<String, dynamic>> _validateAndSanitizeOutputPath(
    ...
    try {
      var sanitizedPath = _sanitizeFilePath(outputPath);
      var outputFile = File(sanitizedPath);
      ...
      if (!await outputDir.exists()) {
        try {
          ...
          // In snap strict confinement, may not be able to create dirs outside $HOME
          if (SnapEnvironment.isRunningInSnap) {
            final home = SnapEnvironment.homeDirectory;
            final fileName = sanitizedPath.split('/').last;
            sanitizedPath = '$home/$fileName';
            outputFile = File(sanitizedPath);
      ...
      try {
        ...
        // In snap, fallback to $HOME if write test fails
        if (SnapEnvironment.isRunningInSnap) {
          final home = SnapEnvironment.homeDirectory;
          final fileName = sanitizedPath.split('/').last;
          sanitizedPath = '$home/$fileName';
