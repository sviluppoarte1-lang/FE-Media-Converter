...
class DRUNetService {
  ...
  static String get _setupScriptPath => path.join(_getScriptsDirectory(), 'setup_python_env.sh');
  ...
  /// Try automatic Python env setup for DRUNet dependencies.
  static Future<Map<String, dynamic>> installDependencies({
    ...
    try {
      ...
      if (!await setup.exists()) {
        return {'success': false, 'error': 'setup_python_env.sh not found'};
      }
      ...
      if (r.exitCode != 0) {
        return {
          'success': false,
          'error': r.stderr.toString().trim().isNotEmpty
              ? r.stderr.toString()
              : r.stdout.toString(),
        };
      }
  ...
  /// Denoise multiple frames: copia in una cartella temporanea e un solo processo Python (--batch-dir).
  ...
  static Future<double?> _probeVideoFps(String inputVideo) async {
    try {
      final r = await Process.run(FFmpegPaths.ffprobePath, [
        '-v', '0',
        '-of', 'csv=p=0',
        '-select_streams', 'v:0',
  ...
  /// Runs ffmpeg; drains stdout; streams stderr lines to [onStderrLine]. Returns exit code.
  ...
  /// Pre-pass: estrae fotogrammi, denoise (OpenCV / script), rimux con audio originale.
  static Future<Map<String, dynamic>> denoiseVideoFull({
    ...
    try {
      ...
      final encoders = await Process.run(FFmpegPaths.ffmpegPath, ['-hide_banner', '-encoders']);
  ...
  /// Process a single frame with the specified mode
  ...
  /// Full video processing pipeline with mode selection
  static Future<Map<String, dynamic>> processVideoFull({
    ...
    try {
      ...
      final extractCmd = [
        FFmpegPaths.ffmpegPath, '-y', '-hwaccel', 'auto', '-i', inputVideo,
        '-vsync', '0',
  ...
  /// Convenience method: upscale video
  ...
  /// Convenience method: deblur video
  ...
  /// Convenience method: restore JPEG artifacts in video
  ...
  /// Process image with DRUNet (single image)
