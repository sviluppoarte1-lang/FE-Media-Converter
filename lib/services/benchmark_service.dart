...
class BenchmarkService {
  ...
  static Future<Map<String, String>> getSystemSignature({
    ...
    final ffmpegInfo = await Process.run(FFmpegPaths.ffmpegPath, ['-hide_banner', '-version']);
    final ffmpegLine = ffmpegInfo.stdout.toString().split('\n').first.trim();
    final driver = await _readDriverSignature();

    final encoderDump = await Process.run(FFmpegPaths.ffmpegPath, ['-hide_banner', '-encoders']);
  ...
  static Future<double> _runCase(
    ...
    final r = await Process.run(FFmpegPaths.ffmpegPath, args);
    sw.stop();
